...
mkfs.fat -n "ORNGOS" $TARGET_PARTITION >/dev/null
...
# copy files
echo "Copying files to ${TARGET_PARTITION} (mounted on /tmp/orngos)..."
mkdir -p /tmp/orngos
mount ${TARGET_PARTITION} /tmp/orngos
cp ${BUILD_DIR}/kernel.bin /tmp/orngos
cp test.txt /tmp/orngos
mkdir /tmp/orngos/test
cp test.txt /tmp/orngos/test
umount /tmp/orngos
